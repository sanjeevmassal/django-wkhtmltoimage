...
from .utils import (content_disposition_filename, render_image_from_template)


class ImageResponse(HttpResponse):
    """HttpResponse that sets the headers for Image output."""
    ...
    def __init__(self, content, status=200, content_type=None,
        ...
        super(ImageResponse, self).__init__(content=content,
                                          status=status,
...
class ImageTemplateResponse(TemplateResponse, ImageResponse):
    """Renders a Template into a Image using wkhtmltoimage"""
    ...
    def __init__(self, request, template, context=None,
        ...
        super(ImageTemplateResponse, self).__init__(request=request,
                                                  template=template,
                                                  context=context,
                                                  status=status,
                                                  content_type=content_type,
    ...
    def rendered_content(self):
        ...
        described by the ImageResponse.
        ...
        return render_image_from_template(
...
class ImageTemplateView(TemplateView):
    """Class-based view for HTML templates rendered to Image."""

    # Filename for downloaded Image. If None, the response is inline.
    ...
    # TemplateResponse classes for Image and HTML
    response_class = ImageTemplateResponse
    html_response_class = TemplateResponse

    # Command-line options to pass to wkhtmltoimage
    cmd_options = {
        
    }

    def __init__(self, *args, **kwargs):
        super(ImageTemplateView, self).__init__(*args, **kwargs)
    ...
    def get(self, request, *args, **kwargs):
        ...
        try:
            ...
            return super(ImageTemplateView, self).get(request,
                                                    *args, **kwargs)
        finally:
    ...
    def render_to_response(self, context, **response_kwargs):
        """
        Returns a IMAGE response with a template rendered with the given context.
        """
        filename = response_kwargs.pop('filename', None)
        cmd_options = response_kwargs.pop('cmd_options', None)

        if issubclass(self.response_class, ImageTemplateResponse):
            if filename is None:
            ...
            return super(ImageTemplateView, self).render_to_response(
