...
def wkhtmltoimage(pages, output=None, **kwargs):
    """
    ...
    **kwargs: Passed to wkhtmltopdf via _extra_args() (See
    ...
    # Default options:
    options = getattr(settings, 'WKHTMLTOIMAGE_OPTIONS', None)
    if options is None:
        options = {'quiet': True}
    else:
    ...
    env = getattr(settings, 'WKHTMLTOIMAGE_ENV', None)
    if env is not None:
        env = dict(os.environ, **env)

    cmd = 'WKHTMLTOIMAGE_CMD'
    cmd = getattr(settings, cmd, os.environ.get(cmd, 'wkhtmltoimage'))
...
class RenderedFile(object):
    ...
    def __init__(self, template, context, request=None):
        debug = getattr(settings, 'WKHTMLTOIMAGE_DEBUG', settings.DEBUG)

        self.temporary_file = render_to_temporary_file(
            template=template,
            context=context,
            request=request,
            prefix='wkhtmltoimage', suffix='.html',
            delete=(not debug)
        )
...
def convert_to_image(filename, header_filename=None, footer_filename=None, cmd_options=None):
    ...
    return wkhtmltoimage(pages=filename, **cmd_options)


def render_image_from_template(input_template, header_template,
                                footer_template, context, request=None, cmd_options=None):
    ...
    return convert_to_image(filename=input_file.filename,
